...
def transfer(payment_information, connection_params):
    client, error = _get_client(**connection_params), None
    ...
    try:
        ...
        stripe_charge = client.Charge.retrieve(payment_information['token'])
        response = {}
        if payment_information['transfer_from_balance']:
            response = client.Transfer.create(
                ...
                metadata={'order_id': payment_information['order_id']},
                destination=payment_information['destination'],
                transfer_group=payment_information['transfer_group'],
            )
        else:
            response = client.Transfer.create(
                amount=stripe_amount,
    ...
    return _create_response(
        payment_information=payment_information,
        kind=TransactionKind.TRANSFER, response=response, error=error)
...
def _get_stripe_charge_payload(
    ...
    shipping = payment_information.get('shipping', None)
...
def _create_response(payment_information, kind, response, error):
    ...
    is_success = (response.get('status') == 'succeeded' or response.get('balance_transaction') != None)
